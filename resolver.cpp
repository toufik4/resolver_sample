...
public:
    ...
    void add_host(std::string host, std::string service = "http")
    {
        ...
        hosts_.emplace_back(std::make_shared<Request>(std::move(host), std::move(service)));
...
private:
    void resolve_next()
    {
        ...
        auto self = shared_from_this();
        auto& host = req->host_;
        auto& service = req->service_;
        std::cout << "🌐 Résolution: " << req->host_ << std::endl;
        ...
        auto lam_solve = [this, self, req](const boost::system::error_code &ec,
                                            tcp::resolver::results_type results)
        {
            ...
            {
                std::cerr << "⏳ Timeout atteint ou résolution annulée pour " << req->host_ << "\n";
            }
            else if (ec == boost::asio::error::host_not_found)
            {
                std::cerr << "❌ Hôte introuvable : " << req->host_ << " (erreur DNS immédiate)\n";
            }
            else if (ec)
            {
                std::cerr << "❌ Autre échec DNS pour " << req->host_ << ": " << ec.message() << "\n";
            }
            else
            {
                std::cout << "✅ Résultat DNS pour " << req->host_ << " :\n";
                for (auto const& r : results)
                {
                    std::string ip = r.endpoint().address().to_string();
                    std::cout << "  -> " << ip << "\n";
                    req->add_ip(ip);
                }
            }
    ...
    std::deque<std::shared_ptr<Request>> hosts_;
    std::mutex mutex_;
    std::thread runner_;
